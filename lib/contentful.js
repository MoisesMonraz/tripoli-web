...
import { getAuthorSlugByName } from './authors';
...
function transformArticle(entry) {
  ...
  const image4 = Array.isArray(fields.image4)
    ? fields.image4.map((img) => extractImage(img)).filter((img) => img.url)
    : [];
  ...
  return {
    ...
    slug: (fields.slug ? fields.slug.replace(/^slug:\s*/i, '').trim() : null) || slugify(fields.title || sys.id),
    ...
    author: (() => {
      ...
      if (authorEntry?.fields) {
        ...
        let cmsName = authorEntry.fields.name || 'Tripoli Publishing House';
        const cmsSlug = authorEntry.fields.slug || null;

        // Name overrides for renamed authors
        if (cmsSlug === "izcoatl-sanchez-patino" ||
            cmsName === "Izcóatl Sánchez" || cmsName === "Izcoatl Sanchez") {
          cmsName = "Gonzalo Sánchez Patiño";
        }

        // Priority: Registry mapping > CMS slug
        // This ensures renamed authors redirect to their new slug
        const finalSlug = getAuthorSlugByName(cmsName) || (
          cmsSlug === "moises-monraz" ? "moises-monraz-escoto" :
          cmsSlug === "izcoatl-sanchez-patino" ? "gonzalo-sanchez-patino" :
          cmsSlug
        );
      ...
      return {
        name: authorName,
        slug: getAuthorSlugByName(authorName),
        role: null,
        bio: null,
        photo: null,
      };
...
export async function getArticlesByAuthorSlug(authorSlug, limit = 200) {
  ...
  if (!client) return [];
  try {
    // Primary: fetch articles linked via Contentful Author reference
    ...
    let linkedArticles = [];
    if (authorResponse.items.length) {
      ...
      linkedArticles = response.items.map(transformArticle);
    }

    // Fallback: also fetch all articles and include those matched via legacy authorMap
    ...
    const legacyArticles = allResponse.items
      .map(transformArticle)
      .filter(a => a.author?.slug === authorSlug);

    // Merge, deduplicate by slug
    const seen = new Set(linkedArticles.map(a => a.slug));
    const merged = [...linkedArticles, ...legacyArticles.filter(a => !seen.has(a.slug))];
