...
/**
 * ⚠️ LOCALE MAPPING — INVERTED BY DESIGN (Legacy Contentful Setup)
 *
 * IMPORTANT: The locale codes below are INTENTIONALLY swapped.
 * When Contentful was first set up, the default locale was 'en-US',
 * but all content was written in Spanish. This cannot be changed
 * without re-creating the entire Contentful space.
 *
 * In practice:
 *   - App language "ES" → Contentful locale 'en-US' (Spanish content)
 *   - App language "EN" → Contentful locale 'es'   (English content, if added)
 *
 * Do NOT "fix" this mapping — it matches the actual Contentful data.
 */
export const LOCALE_MAP = {
  ES: 'en-US',  // Spanish content — stored in Contentful's 'en-US' locale (legacy)
  EN: 'es',     // English content — stored in Contentful's 'es' locale
};
 ...
 * Uses Contentful's full-text query to narrow results, then matches by slugified title.
...
export async function getArticleBySlug(slug, language = 'ES') {
  ...
  try {
    ...
    if (response.items.length === 10) {
      console.warn(`[getArticleBySlug] Query "${query}" returned max results (10). Slug match may be missed if the target article was not in the first 10 results.`);
    }
