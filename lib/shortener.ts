...
export async function createShortURL(originalUrl: string): Promise<ShortURL | null> {
  if (!isValidTripoliURL(originalUrl)) {
    return null;
  }
  ...
  let exists = await kv.get(`url:${code}`);
  while (exists) {
    code = generateCode();
    exists = await kv.get(`url:${code}`);
  }
  ...
  await kv.set(`url:${code}`, shortURL);
  await kv.lpush('url:codes', code);
...
export async function incrementClicks(code: string): Promise<void> {
  const data = await kv.get<ShortURL>(`url:${code}`);
  if (data) {
    data.clicks += 1;
    await kv.set(`url:${code}`, data);
  }
}

// Get all shortened URLs sorted newest first
export async function getAllShortURLs(): Promise<ShortURL[]> {
  const codes = await kv.lrange('url:codes', 0, -1);
  const urls: ShortURL[] = [];
...
export async function deleteShortURL(code: string): Promise<boolean> {
  const exists = await kv.get(`url:${code}`);
  if (!exists) return false;

  await kv.del(`url:${code}`);
  await kv.lrem('url:codes', 1, code);
  return true;
}
