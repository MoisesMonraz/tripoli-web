            ...
            >
              ...
              >
                <div className="flex flex-col gap-1">
                  <h2 className="text-lg lg:text-xl font-semibold text-slate-800 dark:text-slate-50">{title}</h2>
                  <p className="text-sm text-slate-600 dark:text-slate-300 max-w-3xl">{tagline}</p>
                  {description ? <p className="text-sm text-slate-600 dark:text-slate-300 max-w-3xl">{description}</p> : null}
                </div>
                <span
                  className={`flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-slate-50 text-slate-700 transition dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 ${
                    isOpen ? "rotate-45" : ""
                  }`}
                  aria-hidden="true"
                >
              ...
              {isOpen && (
                <div className="px-4 sm:px-5 pb-5">
                  <div className="flex flex-col gap-3 border-t border-slate-200/80 pt-4 dark:border-slate-800">
                    <p className="text-xs uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">{introLabel}</p>
                    <ul className="flex flex-col gap-2 text-sm text-slate-700 dark:text-slate-200 leading-relaxed">
                      {bullets.map((item) => (
                        <li key={item} className="flex gap-2">
                          <span className="mt-[6px] h-[6px] w-[6px] rounded-full bg-sky-500" aria-hidden="true" />
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
                    {closing ? <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{closing}</p> : null}
                  </div>
                </div>
              )}
            </AnimatedServiceBorderBox>
